function showDrawer()
{
    $("#drawer").show();
    $("#ctn").hide();
}

function showProfile()
{
    $("#drawer").hide();
    $("#ctn").show();
    $("#ctn").load('view/Member/Profile.html');
}
            
function showDashboardMember(id){
    var dataMember = "getMember="+id;
    $.ajax({
        type: 'POST',
        url: 'app/Member.php',
        data: dataMember,
        success : function(data){
            var member = $.parseJSON(data);
            $("#navMemberName").text(member.first_name+' '+member.last_name);
            $("#navMemberName-ph").text(member.first_name+' '+member.last_name);
            showDashboardMemberIcon(member.id);
        }
    });
}

function showDashboardMemberIcon(id){
    var dataMember = "getMember="+id;
    $.ajax({
        type: 'POST',
        url: 'app/Member.php',
        data: dataMember,
        success : function(data){
            var member = $.parseJSON(data);
            var src;
            if(member.avatar===null){
                src = 'data:image/png;base64,';
            } else {
                getMemberAvatarURL(member.avatar,function(avadata){
                    src = 'assets/members/avatar/'+avadata.src;
                    $("#navMemberIcon").attr('src',src);
                    $("#navMemberIcon-ph").attr('src',src);
                });
            }
            $("#navMemberIcon").attr('src',src);
            $("#navMemberIcon-ph").attr('src',src);
        }
    });
}

function getMemberAvatarURL(avatar,handleData){
    var dataAvatar = "getAvatar="+avatar;
    $.ajax({
        type: 'POST',
        url: 'app/Member.php',
        data: dataAvatar,
        success : function(data){
            var avatar = $.parseJSON(data);
            handleData(avatar);
        }
    });

}

function getMemberAddress(address,handleData){
    var dataAddress = "getAddress="+address;
    $.ajax({
        type: 'POST',
        url: 'app/Member.php',
        data: dataAddress,
        success : function(data){
            var address = $.parseJSON(data);
            handleData(address);
        }
    });

}

function startInet()
{
    //console.log(loginData);
    $.ajax({
        type: 'POST',
        url: 'http://portal.duwifi.net/login',
        data: loginData,
        success: function(data){
            console.log(data);
        }
    });

    var loginHotspot = "session_hotspot_login=online";
    $.ajax({
        type: 'POST',
        url: 'app/Session.php',
        data: loginHotspot
    });
    onlineInetState();
    console.log('Test Start Hotspot Sukses');
}
function stopInet()
{
    $.ajax({
        type: 'POST',
        url: 'http://portal.duwifi.net/logout',
        success: function(data){
            console.log(data);
        }
    });

    var logoutHotspot = "session_hotspot_login=offline";
    $.ajax({
        type: 'POST',
        url: 'app/Session.php',
        data: logoutHotspot
    });
    offlineInetState();
    console.log('Test Stop Hotspot Sukses');
}

function onlineInetState()
{
    $("#inetBtnState").text('STOP KONEKSI INTERNET');
    $("#inetBtn").attr('class','tile double bg-lightRed fg-white');
    $("#inetBtn").attr('onclick','stopInet();return false');
}
function offlineInetState()
{
    leftSessionTime(sessionRadUser);
    $("#inetBtn").attr('class','tile double bg-lightGreen fg-white');
    $("#inetBtn").attr('onclick','startInet();return false');
}

function leftSessionTime(username)
{
    var dataTime = "checkLeftTime=yes&username="+username;
    $.ajax({
        type: 'POST',
        url: 'app/Radius.php',
        data: dataTime,
        success: function(data){
            var time = $.parseJSON(data);
            var text = "Sisa Waktu : "+time.hours+" Jam "+time.minutes+" Menit "+time.seconds+" Detik";
            $("#inetBtnState").text(text);
        }
    });
}
